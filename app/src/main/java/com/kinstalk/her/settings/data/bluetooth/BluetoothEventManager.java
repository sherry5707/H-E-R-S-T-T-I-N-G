...
//import com.android.settings.qlove.bt.btListActivity.QloveBtListActivity;

/**
 * BluetoothEventManager receives broadcasts and callbacks from the Bluetooth API and dispatches the
 * event on the UI thread to the right class in the Settings.
 */
public final class BluetoothEventManager {
    ...
    private void setDefaultBtName() {
        String name = mContext.getResources().getString(R.string.bt_device_name);
        String sn = getSerialNumber();
        if(!TextUtils.isEmpty(sn) && (sn.length() >= 4)) {
            name = name + "-" + sn.substring(sn.length() - 4);
        }
        Log.d(TAG, "Customized Bluetooth name: " + name);
        ...
        //    Log.d(TAG, "Bluetooth is opened, Begin to start scanning...");
        //}
    }
    ...
    private class AdapterStateChangedHandler implements Handler {
        public void onReceive(Context context, Intent intent,
            ...
            Log.d(TAG, "AdapterStateChangedHandler, intent=" + intent + " state=" + state);
        ...
        public void onReceive(Context context, Intent intent,
            ...
            if (cachedDevice == null) {
                ...
                Log.d(TAG, "DeviceFoundHandler created new CachedBluetoothDevice: "
                        + cachedDevice.getName() + "  " + cachedDevice.getAddress());
    ...
    private class BondStateChangedHandler implements Handler {
        public void onReceive(Context context, Intent intent,
            ...
            int bondPreState = intent.getIntExtra(BluetoothDevice.EXTRA_PREVIOUS_BOND_STATE,
                    BluetoothDevice.ERROR);
            Log.d(TAG, "BondStateChangedHandler.onReceive: device = " + device
                    + ", bondState "+ bondState);
            ...
            } else if((bondPreState != BluetoothDevice.BOND_BONDED) && (bondState == BluetoothDevice.BOND_BONDED)) {
                //this device is added somewhere else before bonding or bonded
                dispatchDeviceAdded(cachedDevice);
            }

            synchronized (mCallbacks) {
                for (BluetoothCallback callback : mCallbacks) {
                    callback.onDeviceBondStateChanged(cachedDevice, bondState, bondPreState);
                }
            }
         ...
         * 
         * @param reason one of the error reasons from BluetoothDevice.UNBOND_REASON_*
         */
...
//            }
    ...
    private class DockEventHandler implements Handler {
        public void onReceive(Context context, Intent intent, BluetoothDevice device) {
            Log.d(TAG, "DockEventHandler.onReceive");
    ...
    boolean readPairedDevices() {
        ...
        for (BluetoothDevice device : bondedDevices) {
            ...
            if (cachedDevice == null) {
                cachedDevice = mDeviceManager.addDevice(mLocalAdapter, mProfileManager, device);
                Log.d(TAG, "readPairedDevices : " + cachedDevice.getName());
                dispatchDeviceAdded(cachedDevice);
                deviceAdded = true;
            }
        }
